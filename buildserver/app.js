    ...
    if (monitorMetrics != null) {
      ...
      if (arr.length > 0) {
        ...
        client.get("dockerMemory", function (err, reply) {
          if (err) throw err;
          var maxMem = parseInt(reply);
          var cpu = arr[0];
          var mem = arr[1];
          var child;
          if (parseInt(cpu) > 30.0 && parseInt(mem) > 50.0) {
            emailContent = "Both the CPU and Memory usages are above threshold limits: " + cpu + "% and " + mem + "%.";
            client.set("dockerMemory", maxMem * 2);
            // Push to production
            fs.appendFileSync(serverLogFilePath, '\nMemory usage high, increasing docker memory and pushing to production.\n');
            child = execSync("./scripts/push_prod_dev.sh " + (maxMem * 2) + "M");
            fs.appendFileSync(serverLogFilePath, '\nOutput in stdout:\n ' + child);
            fs.appendFileSync(serverLogFilePath, 'dev branch pushed to production.\n');
            fs.appendFileSync(serverLogFilePath, '\nPushing to production\n');
            child = execSync("./scripts/push_prod_release.sh " + (maxMem * 2) + "M");
            fs.appendFileSync(serverLogFilePath, '\nOutput in stdout:\n ' + child);
            fs.appendFileSync(serverLogFilePath, 'release branch pushed to production.\n');
          }
          else if (parseInt(cpu) > 30.0) {
            emailContent = "CPU usage is above threshold";
          }
          else if (parseInt(mem) > 50.0) {
            emailContent = "Memory usage is above threshold limit: " + mem + "%.";
            client.set("dockerMemory", maxMem * 2);
            // Push to production
            fs.appendFileSync(serverLogFilePath, '\nMemory usage high, increasing docker memory and pushing to production.\n');
            child = execSync("./scripts/push_prod_dev.sh " + (maxMem * 2) + "M");
